...
def test_complete_pipeline():
    ...
    context = ResearchContext(
        ...
        recommendations=["Verify async performance benchmarks", "Check PEP documentation"],
        ...
        metadata={
            "version": "1.0",
            "research_duration": 2.3
        }
    )
    
    print(f"  Created ResearchContext with:")
    ...
    assert abs(context.confidence - expected_confidence) < 0.01, "Confidence calculation incorrect"
    print(f"  ✓ Confidence calculation correct (expected: {expected_confidence:.2f}, got: {context.confidence:.2f})")
