...
def main():
    ...
    parser.add_argument('--output', type=str, default='docs/architecture', help='Output directory for generated files')
    parser.add_argument('--format', type=str, default='dot,mermaid,drawio,json,markdown,report', help='Output formats to generate')
    parser.add_argument('--visuals', action='store_true', help='Generate visual diagrams (PNG, SVG, PDF) using Graphviz')
    ...
    try:
        ...
        print(f"  [OK] Successfully categorized all {len(analyzer.graph.modules)} modules into 7 layers.")
        ...
        saved_files = output_generator.save_all_outputs(output_dir, include_visuals=args.visuals)
        ...
        print(f"    - {output_dir / 'architecture_flow.mmd'} (Cognitive Execution Flow Diagram)")
        print(f"    - {output_dir / 'architecture.drawio'} (Native Draw.io XML Format)")
        print(f"    - {output_dir / 'architecture.json'} (Structured Machine JSON Data)")
        print(f"    - {output_dir / 'ARCHITECTURE.md'} (System Architecture Documentation)")
        print(f"    - {output_dir / 'architecture_report.txt'} (Human-Readable Analysis Report)")
