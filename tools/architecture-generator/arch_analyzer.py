...
from ast_parser import ModuleInfo, ImportInfo
from config import ArchitectureConfig, ArchitectureLayer, LAYERS_BY_LEVEL
...
class ArchitectureGraph:
    ...
    modules: Dict[str, LayerModule] = field(default_factory=dict)         # module_name -> LayerModule
    layers: Dict[str, List[LayerModule]] = field(default_factory=dict)     # layer_name -> list of LayerModules
    dependencies: List[ModuleDependency] = field(default_factory=list)
    violations: List[dict] = field(default_factory=list)
    module_map: Dict[str, str] = field(default_factory=dict)              # module_name -> layer_name
...
class ArchitectureAnalyzer:
    ...
    def _classify_role(self, module_name: str, module_info: ModuleInfo) -> str:
        ...
        # Check Orchestrator
        if "orchestrator" in name_lower or any("orchestrator" in c for c in class_names) or "auracore" in name_lower or "executive" in name_lower:
            return "ORCHESTRATOR"
            
        # Check Blackboard / CognitiveState
        if "blackboard" in name_lower or "cognitivestate" in name_lower or "decisioncontext" in name_lower:
            return "BLACKBOARD"
        ...
        if "engine" in name_lower or "planner" in name_lower or any("engine" in c or "planner" in c for c in class_names):
            return "ENGINE"
    ...
    def _resolve_module(self, import_module: str, current_module: str) -> Optional[str]:
        ...
        if import_module.startswith('.'):
            ...
            path_parts = list(current_module_path.parts[:-level]) if level > 0 else list(current_module_path.parts)
        ...
        # Handle absolute imports
            ...
            for m_name in all_mods.keys():
                if m_name == first_part or m_name == last_part or m_name.endswith(f".{last_part}"):
                    return m_name
