...
class OutputGenerator:
    ...
    def generate_dot(self, output_path: Path | None = None) -> str:
        content = self.graph_generator.generate_dot(format="dot")
        if output_path:
            self._save_file(output_path, content)
        return content

    def generate_mermaid(self, output_path: Path | None = None) -> str:
        content = self.graph_generator.generate_mermaid(format="mermaid")
        if output_path:
            self._save_file(output_path, content)
        return content

    def generate_cognitive_mermaid(self, output_path: Path | None = None) -> str:
        content = self.graph_generator.generate_cognitive_flow_mermaid()
        if output_path:
            self._save_file(output_path, content)
        return content

    def generate_drawio(self, output_path: Path | None = None) -> str:
    ...
    def generate_markdown_docs(self, output_path: Path | None = None) -> str:
    ...
    def generate_statistics_report(self, output_path: Path | None = None) -> str:
        stats = self.graph_generator.generate_statistics()
        ...
        for layer_config in ArchitectureConfig.ALL_LAYERS:
            ...
            lines.append(
                f"  {layer_config.icon} {layer_name} (Level {layer_config.level}):"
            )
            ...
            lines.append(
                f"    - Functions Count: {layer_data.get('function_count', 0)}"
            )
            lines.append(f"    - Complexity Score: {layer_data.get('complexity', 0)}")
            lines.append("")

        lines.extend(["-" * 80, "  EXPECTED DEPENDENCY CHAIN", "-" * 80, ""])
    ...
    def save_all_outputs(self, output_dir: Path, include_visuals: bool = False) -> dict:
        ...
        if include_visuals:
            results["png"] = self._render_graphviz(
                Path(results["dot"]), output_dir / "architecture.png", "png"
            )
            results["svg"] = self._render_graphviz(
                Path(results["dot"]), output_dir / "architecture.svg", "svg"
            )
            results["pdf"] = self._render_graphviz(
                Path(results["dot"]), output_dir / "architecture.pdf", "pdf"
            )

        return results

    def _render_graphviz(
        self, dot_file: Path, output_file: Path, fmt: str
    ) -> str | None:
        try:
