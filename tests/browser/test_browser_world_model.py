...
import pytest
from src.browser.world_model import BrowserTab, BrowserWorldModel, BrowserStateProbe
...
def test_browser_world_model_search():
    tabs = [
        BrowserTab(tab_id="1", title="Instagram", domain="instagram.com", browser_name="chrome"),
        BrowserTab(tab_id="2", title="GitHub · Repository", domain="github.com", browser_name="edge"),
        BrowserTab(tab_id="3", title="ChatGPT", domain="chatgpt.com", browser_name="firefox"),
    ]
    model = BrowserWorldModel(running_browsers=["chrome", "edge", "firefox"], open_tabs=tabs)
...
def test_browser_state_probe_title_inference():
    domain_insta = BrowserStateProbe._infer_domain_from_title("Instagram - Google Chrome")
    domain_github = BrowserStateProbe._infer_domain_from_title("GitHub · SoulSeeker07/AuraAI - Microsoft Edge")
    domain_chatgpt = BrowserStateProbe._infer_domain_from_title("ChatGPT — Mozilla Firefox")
...
def test_planner_reuse_existing_tab():
    ...
    existing_tabs = [
        BrowserTab(tab_id="win_101", title="Instagram", domain="instagram.com", browser_name="chrome")
    ]
...
def test_planner_close_single_tab_preserves_other_tabs():
    ...
    tabs = [
        BrowserTab(tab_id="1", title="Instagram", domain="instagram.com", browser_name="chrome"),
        BrowserTab(tab_id="2", title="ChatGPT", domain="chatgpt.com", browser_name="chrome"),
    ]
...
def test_planner_close_ambiguity_resolution():
    ...
    tabs = [
        BrowserTab(tab_id="1", title="Instagram", domain="instagram.com", browser_name="chrome"),
        BrowserTab(tab_id="2", title="Instagram", domain="instagram.com", browser_name="edge"),
    ]
...
def test_planner_close_every_instagram_page():
    ...
    plan = planner.create_plan("Close every Instagram page", parameters={"browser_world": world})
