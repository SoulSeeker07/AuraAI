...
def test_resource_ownership_tracker():
    ...
    tracker.register_resource(
        "tab", "tab_101", owner=ResourceOwner.AURA, details={"site": "github"}
    )
    tracker.register_resource(
        "tab", "tab_102", owner=ResourceOwner.USER, details={"site": "instagram"}
    )
...
def test_semantic_category_inference():
    cat_doc = BrowserStateProbe._infer_semantic_category(
        "Python 3.11 Documentation", "docs.python.org"
    )
    cat_shop = BrowserStateProbe._infer_semantic_category(
        "Amazon.com: Online Shopping", "amazon.com"
    )
    cat_social = BrowserStateProbe._infer_semantic_category(
        "Instagram", "instagram.com"
    )
    cat_code = BrowserStateProbe._infer_semantic_category(
...
def test_planner_close_aura_resources_only():
    ...
    plan = planner.create_plan(
        "Close everything you opened", parameters={"browser_world": ctx}
    )
...
def test_planner_close_semantic_documentation_tabs():
    ...
    plan = planner.create_plan(
        "Close all documentation tabs", parameters={"browser_world": ctx}
    )
