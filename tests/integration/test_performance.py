...
def test_startup_time():
    ...
    try:
        ...
        if startup_time < 5.0:
            ...
            print("  ⚠ Startup time is acceptable (< 10 seconds)")
        else:
            print("  ⚠ Startup time is slow (> 10 seconds)")
...
def test_research_latency():
    ...
    try:
        ...
        print("  ✓ Research latency test passed")
        return True
...
def test_memory_usage():
    ...
    try:
        ...
        if process:
            ...
            if mem_mb < 100:
                print("  ✓ Memory usage is low (< 100 MB)")
            elif mem_mb < 500:
                print("  ⚠ Memory usage is moderate (< 500 MB)")
            else:
                print("  ⚠ Memory usage is high (> 500 MB)")
        else:
...
def test_workspace_scan():
    ...
    try:
        ...
        if hasattr(aura_core, "workspace_manager"):
            ...
            print(
                f"  ✓ Workspace scan completed: {files_count} files in {scan_time:.3f} seconds"
            )

            if scan_time < 1.0:
                print("  ✓ Scan is fast (< 1 second)")
            elif scan_time < 3.0:
                print("  ⚠ Scan is moderate (< 3 seconds)")
            else:
                print("  ⚠ Scan is slow (> 3 seconds)")
...
def test_file_operations():
    ...
    try:
        ...
        if read_time < 0.1:
            print("  ✓ File operations are fast")
...
def test_memory_pipeline():
    ...
    try:
        ...
        print(
            f"  ✓ Memory operations speed: {write_time:.3f} seconds for 100 operations"
        )
