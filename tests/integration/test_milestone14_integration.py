...
from src.research.models import ResearchConfig, SearchMode, Evidence, Citation, SourceTrustLevel
...
def test_research_engine_with_planner():
    ...
    try:
        ...
        assert isinstance(result, ResearchContext), f"Expected ResearchContext, got {type(result)}"
...
def test_reasoning_layer():
    ...
    try:
        from src.research.reasoning_layer import ResearchReasoner, ReasoningResult
        ...
        assert reasoning_result.confidence >= 0.0 and reasoning_result.confidence <= 1.0, \
...
def main():
    ...
    # Run all tests
    results.append(("Research Engine with Planner", test_research_engine_with_planner()))
