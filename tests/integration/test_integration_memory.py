...
def test_1_conversation_context():
    ...
    memory.remember_exchange(
        ...
        "Got it! Python is a great choice.",
        "Preference",
    )
...
"""
    ...
    assert (
        "Python" in context or "language" in context.lower()
    ...
    assert recent_messages[0].get("role") in [
        "user",
        "assistant",
...
def test_2_facts():
    ...
    ), f"Expected category 'person', got '{fact.category}'"
...
def test_4_topic_memory():
    ...
    assert (
        len(recent_messages[0].get("content", "")) > 0
    ...
    memory.remember_exchange(
        "Go back to networking.", "Alright, back to networking!", "General"
    )

    recent_messages = manager.get_recent_messages(
        limit=10
    )  # Get more messages to see old ones
...
def test_5_context_size():
    ...
    has_facts = any(
        ...
        for keyword in ["name", "sreekanta", "language", "python", "food", "pizza"]
    )
...
def run_all_integration_tests():
    ...
    print("\n" + "=" * 70)
    print(
        f"INTEGRATION TEST RESULTS: {passed} passed, {failed} failed out of {len(tests)} tests"
    )
    print("=" * 70)
