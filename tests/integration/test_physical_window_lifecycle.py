...
def test_real_notepad_window_lifecycle():
    ...
    try:
        # 1. Launch Notepad
        res_launch = backend.execute("app_open", "Open Notepad", {"app_name": "notepad"})
        ...
        # 3. Minimize Notepad
        res_min = backend.execute("window.minimize", "Minimize Notepad", {"app_name": "notepad"})
        assert res_min.success is True
        time.sleep(0.3)

        # 4. Verify Iconic (Minimized)
        assert bool(win32gui.IsIconic(main_hwnd)) is True, "Expected window to be minimized (IsIconic)"

        # 5. Restore Notepad
        res_restore = backend.execute("window.restore", "Restore Notepad", {"app_name": "notepad"})
        ...
        assert any(not bool(win32gui.IsIconic(h)) for h in notepad_hwnds), "Expected Notepad window to be restored from icon"

        # 7. Close Notepad
        res_close = backend.execute("app_close", "Close Notepad", {"app_name": "notepad"})
        assert res_close.success is True
        time.sleep(1.0)

        # 8. Verify Destroyed / Process Closed
        hwnds_after = policy._get_running_windows("notepad", None)
        assert len(hwnds_after) == 0, f"Expected 0 Notepad windows remaining, found {len(hwnds_after)}"
    finally:
        reset_desktop_execution_engine()
