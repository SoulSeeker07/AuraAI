...
def test_get_recent_messages():
    ...
    assert (
        len(messages) > 0
    ), "✅ CRITICAL: get_recent_messages() should return messages, not empty list!"
...
def test_get_context():
    ...
    assert (
        "John Doe" in context or "name" in context.lower()
...
def test_persistence():
    ...
    # Store a message
    exchange1 = (
        memory1.recent_messages(limit=1)[0]
        if memory1.recent_messages(limit=1)
        else None
    )
...
def test_build_context_in_memory():
    ...
    memory.remember_exchange(
        "What is Aura AI?", "Aura AI is a Python AI operating system", "AI"
    )
    memory.remember_exchange(
        "What are its features?",
        "Features include agent system, memory, automation",
        "Features",
    )
