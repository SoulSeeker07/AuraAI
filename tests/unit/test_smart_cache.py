...
from research.models import SearchMode, SearchQuery, SearchResult, SourceTrustLevel
...
def test_content_type_detection():
    ...
    test_cases = [
        ...
        (
            ...
            "general",
        ),  # General release query (no GitHub context)
        (
            "python release v3.14",
            "github_releases",
        ),  # GitHub release with version number
...
def test_ttl_calculation():
    ...
    for content_type, expected_ttl in test_cases:
        ...
        print(
            f"  {status} Content Type: {content_type:20s} Expected TTL: {expected_ttl:8d}s, Got: {ttl:8d}s"
        )
...
def test_cache_with_content_types():
    ...
    for query_text, expected_type in test_queries:
        ...
        print(f"  {status} Query: '{query_text}'")
        print(
            f"      Expected Type: {expected_type:15s} Got: {cached.get('content_type', 'NOT FOUND')}"
        )
        assert (
            cached.get("content_type") == expected_type
        ), f"Failed for query: {query_text}"
...
def test_ttl_validates_cache_entry():
    ...
    for content_type, ttl, elapsed_time in test_cases:
        ...
        print(
            f"    Entry with future timestamp: {'valid' if valid_future else 'expired'}"
        )
        print(
            f"    Entry with expired timestamp: {'valid' if valid_expired else 'expired (removed)'}"
        )
