    ...
    class SimpleTool:
        ...
        def execute(self, operation, parameters, context=None):
            if operation == "read":
                ...
                return {"status": "success", "written": len(parameters.get("content", ""))}
...
def test_existing_tool_adapter():
    ...
    class TestTool:
        ...
        def get_metadata(self):
            from src.execution import ToolMetadata, ToolCategory
            return ToolMetadata(
                name=self.name,
                description=self.description,
                category=ToolCategory.UTILITY
