...
from brain.models import (
    IntentName, WebSearchResultSimple, ResearchFinding, DeepResearchResult,
...
```
    ...
    def test_extract_pdf_with_pypdf2(self, page_reader, mocker):
        ...
        # Mock PyPDF2
        mocker.patch('src.brain.page_reader.PyPDF2.PdfReader', return_value=mocker.Mock())
...
```
    ...
    async def test_read_pages_parallel(self, page_reader, mocker):
        ...
        assert len(pages) == 3
        assert call_order == [0, 1, 2]  # All should be called concurrently (approximate)
    ...
    def test_intent_detection_web_search(self, mocker):
        ...
        mock_response.choices = [mocker.Mock(
            message=mocker.Mock(
                content=f'{{"intent": "web_search", "confidence": 0.95, "sources": ["python.org"], "subtopics": ["version", "features"]}}'
            )
        )]
        
        mocker.patch('src.brain.intent_analyzer.client.chat.completions.create', return_value=mock_response)
    ...
    def test_intent_detection_code_related(self, mocker):
        ...
        mock_response.choices = [mocker.Mock(
            message=mocker.Mock(
                content=f'{{"intent": "code", "confidence": 0.92, "sources": ["github.com", "stackoverflow.com"], "subtopics": ["useEffect", "hooks", "React"]}}'
            )
        )]
    ...
    def test_intent_detection_coding_help(self, mocker):
        ...
        mock_response.choices = [mocker.Mock(
            message=mocker.Mock(
                content=f'{{"intent": "coding_help", "confidence": 0.98, "sources": ["stackoverflow.com", "github.com"], "subtopics": ["debugging", "error_fix", "code_review"]}}'
            )
        )]
    ...
    def test_source_detection_python_docs(self, mocker):
        ...
        mock_response.choices = [mocker.Mock(
            message=mocker.Mock(
                content=f'{{"intent": "web_search", "confidence": 0.90, "sources": ["python.org"], "subtopics": ["asyncio", "run", "coroutines"]}}'
            )
        )]
    ...
    def test_source_detection_github(self, mocker):
        ...
        mock_response.choices = [mocker.Mock(
            message=mocker.Mock(
                content=f'{{"intent": "code", "confidence": 0.88, "sources": ["github.com"], "subtopics": ["websocket", "server", "python"]}}'
            )
        )]
    ...
    def test_source_detection_programming_sites(self, mocker):
        ...
        mock_response.choices = [mocker.Mock(
            message=mocker.Mock(
                content=f'{{"intent": "web_search", "confidence": 0.87, "sources": ["stackoverflow.com", "w3schools.com"], "subtopics": ["list", "tuple", "difference", "python"]}}'
            )
        )]
        ...
        assert len(analysis.detected_sources) > 0
        assert any("stackoverflow.com" in source or "w3schools.com" in source for source in analysis.detected_sources)
    ...
    def test_citation_builder_format(self):
        ...
        assert '[Python Documentation](https://docs.python.org/3/library/asyncio.html)' in markdown
    ...
    def test_source_ranker_authority_scoring(self):
        ...
        sources = [
            ...
            {'url': 'https://github.com/user/repo', 'relevance': 0.7, 'authority': 'GITHUB'},
        ...
        assert ranked[1]['url'] == 'https://github.com/user/repo'  # GitHub has high score
    ...
    def test_intent_router_coding_help(self):
        ...
        intent = IntentAnalysis(
            intent=IntentAnalysis.INTENT_CODING_HELP,
            confidence=0.98
        )
    ...
    def test_research_progress_callback(self, page_reader):
        ...
        assert len(stages) == 3
        assert all(st['stage'] == 1 and st['completed'] == 3 and st['total'] == 3 for st in stages)
