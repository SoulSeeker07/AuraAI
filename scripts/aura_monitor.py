...
class AuraMonitor:
    ...
    def __init__(self, aura_core: AuraCore | None = None, refresh_interval: int = 2):
        ...
        self._owns_core = (
            aura_core is None
        )  # only standalone mode is allowed to (re)build
    ...
    def _write_component_status(self):
        ...
        self._write_log("\n--- COMPONENT STATUS ---")
        self._write_log(
            f"  Memory: {self._format_status(AuraCoreStatus.READY if self.core.memory_enabled else AuraCoreStatus.ERROR)}"
        )
        self._write_log(
            f"  Knowledge: {self._format_status(AuraCoreStatus.READY if self.core.knowledge_enabled else AuraCoreStatus.ERROR)}"
        )
        self._write_log(
            f"  Workspace: {self._format_status(AuraCoreStatus.READY if self.core.workspace_aware else AuraCoreStatus.ERROR)}"
        )
        self._write_log(
            f"  Plugins: {self._format_status(AuraCoreStatus.READY)} - {self.core.plugin_count} loaded"
        )
        self._write_log(
            f"  Agent Runtime: {self._format_status(self.core.agent_runtime_status)}"
        )
        self._write_log(
            f"  Workflow Engine: {self._format_status(self.core.workflow_engine_status)}"
        )
        self._write_log(
            f"  Vision: {self._format_status(AuraCoreStatus.ERROR if not self.core.vision_enabled else AuraCoreStatus.READY)}"
        )
        self._write_log(
            f"  Voice: {self._format_status(AuraCoreStatus.ERROR if not self.core.voice_enabled else AuraCoreStatus.READY)}"
        )
    ...
    def load_core(self) -> bool:
        ...
        if not self._owns_core:
            logger.warning(
                "load_core() called on a monitor bound to an external AuraCore — ignoring rebuild"
            )
        ...
        try:
            ...
            self.core = AuraCore(
                config={"project_root": project_root, "data_path": data_path}
            )
            return True
