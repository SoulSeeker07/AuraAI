"""
Test Arc Reactor Energy Cycle Flowchart Render & Screenshot
===========================================================
Location: scripts/test_arcreactor_render.py
"""
...
def run():
    ...
    screenshot_path = artifacts_dir / "arcreactor_diagram_render.png"

    def inject():
        overlay._clear_messages()
        prompt = "draw a flowchart of the Iron Man Arc Reactor energy cycle"
        overlay._append_card("user", prompt)

        resp = FastLLMClient.query(prompt)
        overlay._append_card("agent", resp, intent_tag="FLOWCHART")
