...
def create_mock_evidence(
    ...
    evidence = Evidence(
        fact=fact_text, source="mock", trust_level=trust_level, score=score, url=""
    )
...
def test_debug_parameter():
    ...
    print("  ✓ Debug parameter passed correctly")
    ...
    if "debug" in config_dict:
        ...
        print("  ✗ to_dict() missing debug field")
    ...
    if config_restored.debug == original_debug:
        ...
        print("  ✗ from_dict() does not restore debug correctly")
...
def test_research_trace_format():
    ...
    if hasattr(ResearchEngine, "_log_research_trace"):
        print("  ✓ _log_research_trace method exists")

        # Get method signature
        sig = inspect.signature(ResearchEngine._log_research_trace)
        print(f"  ✓ Method signature: {sig}")
    else:
        print("  ✗ _log_research_trace method not found")
        return False
...
def test_planner_refinement():
    ...
    if hasattr(ResearchPlanner, "refine_plan"):
        print("  ✓ refine_plan method exists")

        # Get method signature
        sig = inspect.signature(ResearchPlanner.refine_plan)
        print(f"  ✓ Method signature: {sig}")
    else:
        print("  ✗ refine_plan method not found")
        return False
    ...
    if "Previous Query" in source and "New Query" in source:
        print("  ✓ Logging includes previous and new queries")
    else:
        print("  ✗ Logging missing previous or new queries")
...
def main():
    ...
    try:
        ...
        print(
            "  1. Evidence scoring with component breakdown (_evaluate_quality_detailed)"
        )
        print(
            "  2. Confidence calculation with trust normalization (_calculate_confidence)"
        )
