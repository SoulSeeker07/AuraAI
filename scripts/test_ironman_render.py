"""
Test Live Iron Man Diagram Rendering & Screenshot
==================================================
Location: scripts/test_ironman_render.py
"""
...
def run():
    ...
    screenshot_path = artifacts_dir / "ironman_diagram_render.png"

    def inject_prompt_and_diagram():
        overlay._clear_messages()
        prompt = "draw a detailed architecture diagram of Iron Man's armor subsystems"
        overlay._append_card("user", prompt)

        resp = FastLLMClient.query(prompt)
        overlay._append_card("agent", resp, intent_tag="ARCHITECTURE")
    ...
    QTimer.singleShot(500, inject_prompt_and_diagram)
    app.exec()
