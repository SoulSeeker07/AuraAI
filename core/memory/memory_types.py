...
class MemoryFact:
    ...
    def _encrypt_value(self, value: str, secret_key: str, decrypt: bool = False) -> str:
        ...
        if decrypt:
            ...
            return ''.join(chr(ord(c) ^ ord(secret_key[i % len(secret_key)])) 
    ...
    def __repr__(self) -> str:
        return (f"MemoryFact({self.layer.value}/{self.category.value}: {self.key}="
...
class ConflictResult:
    ...
    resolved: bool
    conflict_fact: Optional[MemoryFact]
    resolution: str
    merged_fact: Optional[MemoryFact]
