...
from .memory_types import (
    MemoryAnalysisResult, CategoryType, ImportanceLevel,
    MemoryLayer
)
...
class MemoryAnalyzer:
    ...
    def _is_too_simple(self, text: str) -> bool:
        ...
        common_words = {'hello', 'hi', 'hey', 'thanks', 'ok', 'yeah', 'no', 'yes', 'um', 'uh'}
    ...
    def _determine_importance(self, text: str) -> ImportanceLevel:
        ...
        # High importance
        high_keywords = ['remember', 'keep', 'save', 'file', 'note', 'always', 'importantly']
    ...
    async def _llm_classify(self, text: str) -> Optional[CategoryType]:
        """
    ...
    def _keyword_classify(self, text: str) -> CategoryType:
        ...
        if any(kw in text_lower for kw in ['like', 'prefer', 'favorite', 'choice', 'enjoy', 'ide']):
            return CategoryType.PREFERENCES

        # Personal (CHECK FIRST to catch personal references)
        if any(kw in text_lower for kw in ['personal', 'remember', 'keep', 'privacy', 'confidential', 'secret']):
            return CategoryType.PERSONAL

        # People-related
        if any(kw in text_lower for kw in ['name', 'person', 'friend', 'colleague', 'manager', 'boss']):
            return CategoryType.PEOPLE

        # Device-related
        if any(kw in text_lower for kw in ['computer', 'laptop', 'phone', 'device', 'screen']):
            return CategoryType.DEVICES

        # Skill-related
        if any(kw in text_lower for kw in ['learn', 'practice', 'skill', 'improve', 'master']):
            return CategoryType.SKILLS

        # Goal-related
        if any(kw in text_lower for kw in ['goal', 'target', 'achieve', 'win', 'success', 'objective']):
        ...
        if 'api' in text_lower and ('endpoint' in text_lower or 'route' in text_lower or 'function' in text_lower or 'class' in text_lower or 'code' in text_lower or 'import' in text_lower):
            return CategoryType.CODING
        # Check for fixing code errors
        if 'fix' in text_lower and ('import' in text_lower or 'error' in text_lower or 'bug' in text_lower):
            return CategoryType.CODING

        # File-related
        if any(kw in text_lower for kw in ['file', 'document', 'report', 'data', 'backup', 'save']):
            return CategoryType.FILES

        # Project-related
        if any(kw in text_lower for kw in ['project', 'task', 'deadline', 'milestone', 'team', 'workflow']):
            return CategoryType.PROJECTS

        # Networking
        if any(kw in text_lower for kw in ['network', 'server', 'cloud', 'database', 'api', 'service']):
