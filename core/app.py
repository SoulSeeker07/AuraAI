...
def create_app():
    ...
    logger.info(f"  - Predefined context requirements loaded: {len(AGENT_CONTEXT_REQUIREMENTS)} types")
    ...
    class AuraApplication:
        ...
        def __init__(self):
            ...
            logger.info(f"  - Total registered agents: {agent_registry.get_agent_count()}")
            logger.info(f"  - Available agent types: {', '.join(agent_registry.get_available_agent_types())}")
        
        async def coordinate_multi_agent_task(self, task: dict, options: dict = None) -> dict:
            """
            ...
            if routing_result.success:
                ...
                orchestration_result = await self.orchestrator.coordinate_task(task, options)
                ...
                logger.info(f"  - Task completed with status: {orchestration_result.success}")
                return result
            else:
        ...
        async def analyze_network(self, network_data: dict) -> dict:
            ...
            """
            task = {
                "task_type": "analyze_network_configuration",
                "data": network_data
            }
            return await self.coordinate_multi_agent_task(task)
