...
12. **M31 Isolated Background Subagents & Non-Blocking Worker Runtime**:
    - `Non-Blocking Dispatch`: Subagents are dispatched via `MasterOrchestrator.dispatch_background_subagent()` using detached `asyncio.create_task()`, returning immediate user acknowledgment in $<50\text{ms}$ while the worker runs asynchronously off-thread.
    - `Context Isolation`: Each `TaskWorker` maintains ephemeral `self.messages` and `TaskWorkingMemory` with bounded turns (`max_turns`) and time budgets (`time_budget_seconds`). Intermediate turns never pollute the primary conversation session or `Blackboard`.
    - `Two-Gate Defense-in-Depth`: Gate 1 evaluates `ExecutionPolicy` prior to dispatch; Gate 2 evaluates policy inside `TaskWorker` before every action. Confirmation-gated actions (`PolicyAction.ASK_USER`) or policy errors strictly fail closed.
    - `Secret Redaction & Notification`: `sanitize_worker_output()` scrubs all provider keys, private keys, bearer tokens, and secrets before output reaches `WorkerResult`, `EventBus` (`subagent.completed` / `subagent.failed`), or `FocusManager` notification cards.
13. **M12 / M15 / TD-020 / TD-021 Interactive Execution & Four-Path Reality**:
    - *Interactive LLM Path (Path C)*: Interactive user requests (`AuraCore.process_request`) early-return to `get_ai_response()` when `llm_enabled=True`. The actual live execution surface is the 15-tool ReAct loop in `AuraToolRegistry`, mediated by `UnifiedToolDispatcher`.
    - *Single Source of Truth*: All tool risks derive strictly from `CapabilityRegistry` $\rightarrow$ `classify_action_risk()` $\rightarrow$ `ExecutionPolicy`. Dynamic action inspection ensures compound tools map destructive verbs (e.g. `close_window`, `terminal.execute`, `browser.submit`) to canonical high-risk capabilities gated by M15 `CryptographicApprovalAuthority` HMAC tickets.
    - *Execution Path Topology*:
      - *Path A (`BackendRegistry` Adapters)*: Dormant for interactive requests when LLM is configured (TD-020).
      - *Path B (`ExecutiveBrain` / ACA)*: Dead (0 call sites across codebase).
      - *Path C (`UnifiedToolDispatcher` $\rightarrow$ `AuraToolRegistry`)*: The sole live interactive execution surface.
      - *Path D (`AmbientContextBuilder`)*: Passive prompt-injection perception reading raw OS primitives directly into LLM system prompts without tool dispatch.