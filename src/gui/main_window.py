...
class MainWindow(QMainWindow):
    ...
    def _setup_titlebar(self):
        ...
        # Window controls
        for sym, color, action in [
            ("−", Colors.TEXT_MUTED, self.showMinimized),
    ...
    def _connect_signals(self):
        app_signals.execution_started.connect(
            lambda _: self._status_indicator.set_active(True)
            or self._status_indicator.set_label("Running")
        )
        app_signals.execution_finished.connect(
            lambda _, __: self._status_indicator.set_active(False)
            or self._status_indicator.set_label("Idle")
        )
