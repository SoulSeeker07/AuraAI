...
class StatusPill(QWidget):
    ...
    def __init__(self, icon: str, label: str, active: bool = False, animate: bool = False, parent=None):
    ...
    def _setup_ui(self):
        ...
        self._icon_label.setStyleSheet("font-size: 12px; background: transparent; border: none;")
        layout.addWidget(self._icon_label)
        
        # Text
        self._text_label = QLabel(self._label)
        self._text_label.setFont(Typography.CAPTION())
        self._text_label.setStyleSheet(f"color: {Colors.TEXT_SECONDARY}; background: transparent; border: none;")
    ...
    def _update_style(self):
        if self._active:
            ...
            """)
            self._text_label.setStyleSheet(f"color: {Colors.CYAN_GLOW}; background: transparent; border: none; font-weight: 600;")
            dot_color = Colors.CYAN if not self._animate else f"rgba(6, 182, 212, {self._pulse_opacity})"
            self._dot.setStyleSheet(f"color: {dot_color}; font-size: 8px; background: transparent; border: none;")
        else:
            self.setStyleSheet(f"""
                StatusPill {{
            ...
            """)
            self._text_label.setStyleSheet(f"color: {Colors.TEXT_MUTED}; background: transparent; border: none;")
            self._dot.setStyleSheet(f"color: {Colors.TEXT_MUTED}; font-size: 6px; background: transparent; border: none;")
    ...
    def pulse_opacity(self, value):
        self._pulse_opacity = value
        if self._active:
            self._dot.setStyleSheet(f"color: rgba(6, 182, 212, {value}); font-size: 8px; background: transparent; border: none;")
