...
class SourceRanker:
    ...
    def _get_authority_score(self, url: str) -> float:
        ...
        domain = (
            url.lower().replace("https://", "").replace("http://", "").split("/")[0]
        )
    ...
    def _calculate_confidence(
        self, authority_score: float, relevance_score: float, url: str
    ) -> float:
        """
    ...
    def _generate_reasoning(
        self, url: str, score: float, authority_score: float
    ) -> list[str]:
        """
    ...
    def select_top_sources(
        self, ranked_results: list[RankedResult], count: int = 3
    ) -> list[RankedResult]:
        """
