...
from brain.request import AuraRequest, ActionType
...
class DecisionEngine:
    ...
    def decide(self, request: AuraRequest, context: Any) -> Decision:
        ...
        if self.agent_registry:
            ...
            task_input = TaskInput(data={"text": request.text, "attachments": request.attachments})
        ...
        if self._is_tool_request(request.text, context):
            ...
            return Decision(action_type=ActionType.TOOL, tool_name=tool_name, params=params)
    ...
    def _is_memory_query(self, text: str, context: Any) -> bool:
        ...
        memory_keywords = [
            "remember", "what do you know", "my facts", "profile", "preferences",
            "summarize", "recall", "retrieve", "what do you remember"
        ]
    ...
    def _is_tool_request(self, text: str, context: Any) -> bool:
        ...
        action_indicators = [
            "open", "close", "create", "delete", "search", "find",
            "read", "write", "save", "analyze", "execute", "run",
            "browse", "navigate", "launch", "start", "stop"
        ]
    ...
    def _detect_tool(self, text: str, context: Any) -> tuple[str, dict]:
        ...
        if "search" in text_lower or "find" in text_lower:
            search_query = text_lower.replace("search for", "").replace("find", "").strip()
        ...
        if len(words) > 0:
            ...
            tool_candidates = [w for w in words if w not in stop_words and not w.endswith('?')]
    ...
    def _is_vision_request(self, text: str, attachments: list) -> bool:
        ...
        text_lower = text.lower().strip()
        vision_keywords = [
            "analyze image", "analyze screenshot", "read document",
            "what's in this", "describe this", "recognize", "ocr",
            "extract text from image", "image recognition"
        ]
...
class Decision:
    ...
    def __init__(self, action_type: ActionType, tool_name: str = "", params: dict = None):
        """
