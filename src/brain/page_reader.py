...
class PageReader:
    ...
    def _remove_elements(self, soup: BeautifulSoup) -> None:
        ...
        for element in soup.find_all(True):
            ...
            if ad_pattern.search(class_names) or nav_pattern.search(class_names) or cookie_pattern.search(class_names):
                element.decompose()
                continue
            
            # Check for ad/affiliate tags
            if element.name in ["ins", "iframe"] and "data-ad" in element.attrs or "adsbygoogle" in element.get("class", []):
                element.decompose()
                continue
    ...
    def _extract_main_text(self, soup: BeautifulSoup) -> str:
        ...
        main_content = (
            ...
            soup.find("main") or
            soup.find("div", class_=re.compile(r"content|main|article", re.IGNORECASE)) or
            soup.find("body")
        )
    ...
    def _extract_github_readme(self, content: str, url: str) -> tuple[str, list[str]]:
        ...
        try:
            ...
            # Extract code blocks
            code_blocks = []
            for match in re.finditer(r'<pre><code(?: class="language-(\w+)")?>(.*?)</code></pre>', html, re.DOTALL):
