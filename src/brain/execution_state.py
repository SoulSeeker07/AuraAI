...
class ExecutionState:
    ...
    current_progress_step: int | None = None
    ...
    def start_task(
        self, task_type: str, title: str, metadata: dict[str, Any] | None = None
    ):
        """
        Start a new task.
    ...
    def update_task_status(self, status: TaskStatus, error: str | None = None):
        """
    ...
    def start_streaming(self):
        ...
        self.stream(
            "idle", "start_streaming", {"conversation_id": self.conversation_id}
        )
    ...
    def update_streaming(self, chunk: str, is_token: bool = False):
        ...
        self.stream(
            "idle",
            "streaming_update",
            {"position": self.streaming_position, "tokens": self.total_tokens},
        )
    ...
    def complete_streaming(self, chunks: int = 0):
        ...
        self.stream(
            "idle",
            "complete_streaming",
            {"chunks": chunks, "tokens": self.total_tokens},
        )
    ...
    def cancel_streaming(self):
        ...
        self.stream(
            "idle", "cancel_streaming", {"conversation_id": self.conversation_id}
        )
    ...
    def add_progress_step(self, step: str):
        ...
        if step not in self.progress_steps:
            ...
            self.stream(
                "idle",
                "progress_step",
                {"step": step, "progress": self.current_progress_step},
            )
            ...
            "current_task": (
                {
                    ...
                    "status": (
                        self.current_task.status.value if self.current_task else None
                    ),
                    "progress": (
                        self.current_task.progress if self.current_task else 0.0
                    ),
