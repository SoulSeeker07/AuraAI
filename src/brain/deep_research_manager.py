...
from brain.citation_builder import CitationBuilder, Citation
...
from brain.source_ranker import SourceRanker, RankedResult
...
class DeepResearchManager:
    ...
    async def _execute_research_plan(
        ...
        is_comparison_query = (
            "compare" in main_query.lower() or 
            "versus" in main_query.lower() or 
            "vs" in main_query.lower()
        )
    ...
    async def _read_top_pages(
        ...
        for i, ranked_result in enumerate(ranked_results[:num_pages]):
            ...
            if on_progress:
                on_progress(f"📄 Reading {ranked_result.result.get('title', 'source')} ({i+1}/{num_pages})...")
    ...
    def _calculate_confidence(self, result: ResearchResult) -> float:
        ...
        avg_confidence = sum(r.confidence for r in result.top_sources[:5]) / min(5, len(result.top_sources))
    ...
    def format_research_summary(
        ...
        if result.top_sources:
            sources_list = ", ".join([r.result.get("title", "") for r in result.top_sources[:5]])
