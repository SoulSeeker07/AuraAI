...
class WorkflowHistory:
    ...
    def log_step_completion(
        ...
        logger.debug(f"Logged step completion {step_id[:8]} in workflow {workflow_id[:8]}")
    ...
    def log_step_failure(
        ...
        logger.error(f"Logged step failure {step_id[:8]} in workflow {workflow_id[:8]}: {error}")
    ...
    def filter_history(
        ...
        if workflow_id:
            filtered = [entry for entry in filtered if entry['workflow_id'] == workflow_id]
        ...
        if trigger_type:
            filtered = [entry for entry in filtered if entry['trigger_type'] == trigger_type]

        # Filter by start time
        if start_time:
            filtered = [entry for entry in filtered if datetime.fromisoformat(entry['start_time']) >= start_time]

        # Filter by end time
        if end_time:
            filtered = [entry for entry in filtered if datetime.fromisoformat(entry['start_time']) <= end_time]

        # Sort by start time (newest first)
        filtered.sort(key=lambda x: datetime.fromisoformat(x['start_time']), reverse=True)
    ...
    def get_statistics(self, workflow_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        durations = [entry['duration'] for entry in filtered if entry['duration'] is not None]
    ...
    def export_history(self, workflow_id: Optional[str] = None, filepath: Optional[str] = None) -> str:
        """
