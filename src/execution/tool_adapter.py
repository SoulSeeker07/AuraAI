...
class BaseToolAdapter(ToolInterface):
    ...
    def get_operation_metadata(self, operation: str) -> dict[str, Any] | None:
...
class ExistingToolAdapter(
    BaseToolAdapter, ProgressReportingAdapterMixin, CancellationSupportMixin
):
    """
    ...
    def _infer_operations(self) -> None:
        ...
        if not self._supported_operations:
            self.add_operation(
                "execute", "Execute the tool", requires_confirmation=False
            )
    ...
    def execute(
        ...
        try:
            ...
            raise ToolExecutionError(
                f"Function execution failed: {str(e)}", original_error=e
            )
