...
OPERATION_RISK = {
    ...
    "browser_extract": RiskLevel.LOW,
    "shopping_search": RiskLevel.LOW,
    "shopping_compare": RiskLevel.LOW,
    "shopping_cart": RiskLevel.MEDIUM,
    "shopping_checkout": RiskLevel.CRITICAL,
    "browser_order": RiskLevel.CRITICAL,
...
class RiskAnalyzer:
    ...
    def analyze_operation(
        self, tool_name: str, operation: str, parameters: dict[str, Any]
    ...
    def _determine_final_risk(
        self, base_risk: RiskLevel, risk_factors: list[str]
    ) -> RiskLevel:
        """
    ...
    def check_if_confirmation_required(
        ...
        """
        risk_level, risk_factors = self.analyze_operation(
            tool_name, operation, parameters
        )
        ...
        if self.require_confirmation:
            ...
            elif risk_level == RiskLevel.MEDIUM and self._has_destructive_factors(
                risk_factors
            ):
                requires_confirmation = True
        ...
        for factor in risk_factors:
            if "force" in factor.lower():
                recommendations.append(
                    "⚠️  Consider using force flag only if absolutely necessary"
                )
            if "multiple" in factor.lower() or "recursive" in factor.lower():
                recommendations.append(
                    "⚠️  Review affected files carefully before proceeding"
                )
            if "system-level" in factor.lower():
                recommendations.append(
                    "⚠️  System-level operations should be reviewed with care"
                )
