...
from .cancellation import CancellationHandler, CancellationToken
...
from .permission_manager import PermissionAction, PermissionContext, PermissionManager
...
class ExecutionEngine:
    ...
    def execute_tool(
        ...
        try:
            ...
            if not is_valid:
                execution_state.fail(
                    ToolValidationError(
                        ...
                        execution_id=execution_id,
                        invalid_params={
                            op: error for op, error in enumerate(error_messages)
                        },
                    )
                )
            ...
            # Check permissions
            required_permission = self.permission_manager.get_required_permission(
                operation
            )
            permission_context.set_permission_level(required_permission)

            execution_state.request_permission(required_permission.value)
            permission_context.check_permission(
                self._map_operation_to_permission_action(operation)
            )
            execution_state.grant_permission(required_permission.value)

            # Analyze risk
            requires_confirmation, risk_level, risk_factors = (
                self.risk_analyzer.check_if_confirmation_required(
                    tool_name=tool_name, operation=operation, parameters=parameters
                )
            )
            ...
            def progress_callback(update):
                ...
                execution_state.add_log(
                    f"Progress: {update.progress}% - {update.message}"
                )
            ...
            cancellation_handler = CancellationHandler(
                cancellation_token=cancellation_token, on_cancel=lambda reason: None
            )
            ...
            if cancellation_token.is_set():
                ...
                return ToolExecutionResult.error_result(
                    error="Operation cancelled", execution_id=execution_id
                )
    ...
    def list_running_executions(self) -> list[dict[str, Any]]:
        ...
        return [
            state.to_dict()
            for state in self.execution_state_manager.get_running_executions()
        ]
