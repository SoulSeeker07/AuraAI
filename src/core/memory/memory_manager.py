...
if __name__ in sys.modules:
    sys.modules.setdefault("core.memory.memory_manager", sys.modules[__name__])
    sys.modules.setdefault("src.core.memory.memory_manager", sys.modules[__name__])
...
class MemoryManager:
    ...
    def remember_fact(self, key: str, value: str, category: str = "general") -> MemoryFact:
        """Store a memory fact key-value pair."""
        return self.remember(category=category, key=key, value=value)

    def get_fact(self, key: str, category: str = "general") -> str | None:
        """Retrieve a memory fact value by key."""
        fact = self.retrieve(category=category, key=key)
    ...
    def build_context(
        ...
        """
        Build context list containing current topic, user input, facts, and conversation history.
        """
        ...
        if facts:
            for f in facts:
                val = f.value if hasattr(f, "value") else str(f)
                cat = f.category if hasattr(f, "category") else "general"
                key_name = getattr(f, "key", "")
                context_parts.append(f"Fact [{cat}] {key_name}: {val}")

        # Add recent conversation messages (bounded by max_messages)
        messages = self.get_recent_messages(limit=min(max_messages, 20))
        for msg in messages:
            role = msg.get("role", "user")
            content = msg.get("content", msg.get("question", msg.get("answer", "")))
            msg_topic = msg.get("topic", topic)
            context_parts.append(f"[{msg_topic}] {role}: {content}")
