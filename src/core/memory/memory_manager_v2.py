...
class MemoryManagerV2:
    ...
    async def analyze_and_remember(
        ...
        if not analysis.should_store:
            logger.debug(
                f"Not storing: {text[:50]}... reason: {analysis.metadata.get('reason')}"
            )
            return None
        ...
        logger.debug(
            f"Checking for existing fact with key '{normalized_key}': {existing_fact is not None}"
        )
        if existing_fact:
            logger.debug(
                f"Updating existing fact: value was '{existing_fact.value}', now '{value}'"
            )
        else:
        ...
        if existing_fact:
            ...
            fact = existing_fact
            logger.info(
                f"Updated memory: {layer.value}/{category.value}/{normalized_key}"
            )
        else:
        ...
        if metadata and metadata.get("contains_sensitive"):
            ...
            logger.info(
                f"Encrypted sensitive memory: {layer.value}/{category.value}/{normalized_key}"
            )
    ...
    async def remember(
        ...
        if existing_fact:
            ...
            self.layers[layer].add_fact(fact)
            logger.info(
                f"Stored memory: {layer.value}/{category.value}/{normalized_key}"
            )
    ...
    async def retrieve_with_reranking(
        self, query: str, layers: list[MemoryLayer] | None = None, limit: int = 10
    ) -> MemoryRetrievalResult:
        """
    ...
    def forget(
        self, key: str, layer: MemoryLayer | None = None, confirm: bool = True
    ) -> ForgettingResult:
        """
        ...
        return ForgettingResult(deleted=deleted, reasons=reasons, warnings=warnings)
    ...
    async def forget_old_memories(
        ...
        for mem_layer in self.layers.values():
            ...
            for fact in mem_layer.facts:
                ...
                if (
                    fact.created_at < cutoff_date
                    and fact.importance.value <= importance_threshold.value
                ):
    ...
    def resolve_conflict(
        self, key: str, new_value: str, layer: MemoryLayer = MemoryLayer.LONG_TERM
    ) -> ConflictResult:
        """
    ...
    def _estimate_storage(self) -> str | None:
    ...
    def _load_memory(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.layers)} layers with "
