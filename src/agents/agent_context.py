    ...
    def _limit_items(self) -> Dict[str, Any]:
        ...
        for key, value in self.filtered_data.items():
            if isinstance(value, list) and len(value) > self.max_items:
                ...
                # Limit dict items
                limited[key] = {
                    k: v for i, (k, v) in enumerate(value.items())
                    if i < self.max_items
                }
            else:
                limited[key] = value
...
class ContextManager:
    ...
    def add_agent_requirement(self, agent_name: str, required_context: List[AgentContextType]) -> None:
        """
    ...
    def summarize_context_usage(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "total_context_types": len(set(
                type_ for req_list in self.agent_requirements.values()
...
AGENT_CONTEXT_REQUIREMENTS = {
    ...
    "VisionAgent": [
        AgentContextType.VISION
    ],
    "VoiceAgent": [
        AgentContextType.VOICE
    ],
    "SecurityAgent": [
        AgentContextType.SECURITY,
        AgentContextType.CONFIGURATION
