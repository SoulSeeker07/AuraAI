...
class PlannerAgent:
    ...
    def analyze_goal(self, user_request: str) -> dict[str, Any]:
        ...
        # Detect goal type
        if any(word in request_lower for word in ["create", "build", "make", "generate", "develop"]):
            goal_type = "creation"
        elif any(word in request_lower for word in ["research", "learn", "find", "understand", "analyze"]):
            goal_type = "research"
        elif any(word in request_lower for word in ["fix", "debug", "error", "bug"]):
            goal_type = "debug"
        elif any(word in request_lower for word in ["organize", "sort", "clean", "manage"]):
        ...
        if any(word in request_lower for word in ["urgent", "important", "immediately"]):
    ...
    def _extract_goal_components(self, request: str) -> List[str]:
        ...
        if any(word in request.lower() for word in ["python", "flask", "django", "web"]):
            components.append("web_development")

        if any(word in request.lower() for word in ["powerpoint", "presentation", "slides"]):
    ...
    def decompose_goal(self, goal_analysis: dict[str, Any], user_context: dict = None) -> List[dict[str, Any]]:
        """
