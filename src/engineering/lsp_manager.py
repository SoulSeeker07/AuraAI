...
class LSPManager:
    ...
    def __init__(self, repository_path: Path, enable_lsp: bool = True):
        """
    ...
    def _configure_servers(self):
        ...
        servers = {
            "python": LSPServer(
                language="python", command="pyright", options={"stdio": True}
            ),
            ...
            "go": LSPServer(language="go", command="gopls", options={"stdio": True}),
            "rust": LSPServer(
                language="rust", command="rust-analyzer", options={"stdio": True}
            ),
        }
    ...
    def get_completions(
        self, file_path: str, line: int, column: int, max_items: int = 50
    ) -> list[dict[str, Any]]:
        """
    ...
    def get_type_at(self, file_path: str, line: int, column: int) -> str | None:
        """
    ...
    def get_hover_info(self, file_path: str, line: int, column: int) -> str | None:
        """
