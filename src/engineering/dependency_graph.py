...
class DependencyGraph:
    ...
    def build_from_files(self, file_paths: Optional[List[Path]] = None):
        ...
        logger.info(f"Dependency graph built with {len(self._dependencies)} dependencies")
    ...
    def _add_dependency(self, dependency: Dependency):
        ...
        source_module = dependency.source.split('.')[0] if '.' in dependency.source else dependency.source
        target_module = dependency.target.split('.')[0] if '.' in dependency.target else dependency.target
    ...
    def _find_cycle_from(self, start_node: str, visited: Optional[Set[str]] = None, 
