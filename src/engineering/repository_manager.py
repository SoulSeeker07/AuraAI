...
class RepositoryManager:
    ...
    def _build_state(self):
        ...
        self._state.size = sum(f.stat().st_size for f in self.repository_path.rglob('*') 
                             if f.is_file())
    ...
    def _detect_language_and_framework(self):
        ...
        max_files = max(len(py_files), len(js_files), len(ts_files), 
                       len(java_files), len(cpp_files), len(go_files), len(rs_files))
        
        if max_files == len(py_files) and py_files:
            self._state.language = "python"
            # Detect framework
            if any('requirements.txt' in str(f) for f in self.repository_path.rglob('*')):
                self._state.framework = "python"
            elif any('pyproject.toml' in str(f) for f in self.repository_path.rglob('*')):
                self._state.framework = "python"
        
        elif max_files == len(ts_files) and ts_files:
            self._state.language = "typescript"
            self._state.framework = "react" if any('react' in f.name.lower() 
                                                  for f in ts_files) else "unknown"
            ...
            self._state.framework = "spring" if any('spring' in f.name.lower() 
                                                   for f in java_files) else "unknown"
    ...
    def _find_test_files(self):
        """Find test files."""
        patterns = [
            'test_*.py', 'tests_*.py', '*_test.py', '*_tests.py',
            'test_*.js', 'test_*.ts', 'spec_*.js', 'spec_*.ts',
    ...
    def _find_documentation(self):
        ...
        doc_patterns = [
            'README.md', 'README.rst', 'CHANGELOG.md', 'changelog.md',
            'HISTORY.md', 'history.md', 'AUTHORS.md', 'docs/',
            'doc/', 'readme/', 'docs/'
        ]
