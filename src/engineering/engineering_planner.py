...
class EngineeringPlanner:
    ...
    def __init__(self, repository_path: Path, use_memory: bool = True):
        """
    ...
    def _generate_refactoring_steps(
        self, operation: str, old_name: str, new_name: str, context: dict[str, Any]
    ) -> list[PlanningStep]:
        ...
        steps.append(
            PlanningStep(
                ...
                details={"strategy": "atomic_commit", "validation": "tests"},
        ...
        steps.append(
            PlanningStep(
                ...
                details={"preparation": "create_backup", "validation": "dry_run"},
        ...
        steps.append(
            PlanningStep(
                ...
                details={"checks": ["code_style", "tests", "documentation"]},
                estimated_time="10-20 minutes",
        ...
        steps.append(
            PlanningStep(
                ...
                details={"tests": context.get("tests", []), "coverage": "target 90%"},
                estimated_time="15-30 minutes",
                dependencies=["Review"],
            )
        )
        ...
        steps.append(
            PlanningStep(
                ...
                details={"changelog": True, "comments": True},
    ...
    def _get_affected_files(
        self, old_name: str, new_name: str, operation: str
    ) -> list[str]:
    ...
    def plan_feature(
        self, feature_name: str, description: str, context: dict[str, Any] | None = None
    ) -> FeaturePlan:
        """
        Plan a new feature.
        ...
        phases = [
            ...
            PlanningStep(
                phase=PlanningPhase.IMPLEMENT, description=f"Implement {feature_name}"
            ),
            PlanningStep(phase=PlanningPhase.TEST, description=f"Test {feature_name}"),
            PlanningStep(
                phase=PlanningPhase.REVIEW, description=f"Review {feature_name}"
            ),
            PlanningStep(
                phase=PlanningPhase.DOCUMENT, description=f"Document {feature_name}"
            ),
            PlanningStep(
                phase=PlanningPhase.COMMIT, description=f"Commit {feature_name}"
            ),
        ]
    ...
    def plan_bug_fix(
        ...
        steps.append(
            PlanningStep(
                phase=PlanningPhase.UNDERSTAND, description="Reproduce the bug"
            )
        )

        # Analyze
        steps.append(
            PlanningStep(phase=PlanningPhase.ANALYZE, description="Analyze root cause")
        )

        # Fix
        steps.append(
            PlanningStep(phase=PlanningPhase.IMPLEMENT, description="Implement fix")
        )
        ...
        # Review
        steps.append(PlanningStep(phase=PlanningPhase.REVIEW, description="Review fix"))

        # Commit
        steps.append(PlanningStep(phase=PlanningPhase.COMMIT, description="Commit fix"))
    ...
    def get_planning_summary(
        self, plan: Union[RefactoringPlan, FeaturePlan]
    ) -> dict[str, Any]:
        """
