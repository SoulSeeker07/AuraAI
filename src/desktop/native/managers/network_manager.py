...
from ..adapters.network_adapter import NetworkAdapterFactory, NetworkAdapter
...
class NetworkManager(BaseNativeManager):
    ...
    def health_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            details = {
                ...
                "internet": "Connected" if internet_info.get("connected") else "Disconnected",
        ...
        **kwargs,
        ...
        try:
            ...
            if cap_clean in ("list_network_interfaces", "network.interfaces"):
                ...
                return self._handle_get_default_interface(goal=goal, capability=capability)
                ...
                return self._handle_get_connection_type(goal=goal, capability=capability)
                ...
                return self._handle_get_signal_strength(goal=goal, capability=capability)

            # Diagnostic Handlers
            elif cap_clean == "network.ping":
                return self._handle_ping(goal=goal, capability=capability, arguments=arguments)
            elif cap_clean == "network.traceroute":
                return self._handle_traceroute(goal=goal, capability=capability, arguments=arguments)
            elif cap_clean == "network.lookup":
                return self._handle_lookup(goal=goal, capability=capability, arguments=arguments)
            elif cap_clean == "network.port_check":
                return self._handle_port_check(goal=goal, capability=capability, arguments=arguments)
                ...
                return self._handle_measure_latency(goal=goal, capability=capability, arguments=arguments)
            elif cap_clean == "network.packet_loss":
                return self._handle_measure_packet_loss(goal=goal, capability=capability, arguments=arguments)

            # Control Handlers
            elif cap_clean == "network.enable_adapter":
                return self._handle_enable_adapter(goal=goal, capability=capability, arguments=arguments)
            elif cap_clean == "network.disable_adapter":
                return self._handle_disable_adapter(goal=goal, capability=capability, arguments=arguments)
            elif cap_clean == "network.release_ip":
                return self._handle_release_ip(goal=goal, capability=capability, arguments=arguments)
            elif cap_clean == "network.renew_ip":
                return self._handle_renew_ip(goal=goal, capability=capability, arguments=arguments)
                ...
                return self._handle_connect_wifi(goal=goal, capability=capability, arguments=arguments)
    ...
    def _handle_get_interfaces(self, goal: str, capability: str) -> DesktopResult:
        ...
        return DesktopResult.create_success(goal=goal, capability=capability, manager=self.name, data={"interfaces": data})

    def _handle_get_default_interface(self, goal: str, capability: str) -> DesktopResult:
        data = self.adapter.get_default_interface()
        return DesktopResult.create_success(goal=goal, capability=capability, manager=self.name, data=data)
    ...
    def _handle_ping(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
    ...
    def _handle_traceroute(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
        host = arguments.get("host", "8.8.8.8")
        max_hops = int(arguments.get("max_hops", 15))
        data = self.adapter.traceroute(host=host, max_hops=max_hops)
        return DesktopResult.create_success(goal=goal, capability=capability, manager=self.name, data=data)

    def _handle_lookup(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
        domain = arguments.get("domain", "google.com")
        data = self.adapter.lookup(domain=domain)
        return DesktopResult.create_success(goal=goal, capability=capability, manager=self.name, data=data)

    def _handle_port_check(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
        host = arguments.get("host", "8.8.8.8")
    ...
    def _handle_measure_latency(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
        host = arguments.get("host", "8.8.8.8")
        data = self.adapter.measure_latency(host=host)
        return DesktopResult.create_success(goal=goal, capability=capability, manager=self.name, data=data)

    def _handle_measure_packet_loss(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
        host = arguments.get("host", "8.8.8.8")
    ...
    def _handle_enable_adapter(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
        ...
        if ok:
            return DesktopResult.create_success(
                goal=goal, capability=capability, manager=self.name,
                data={"status": "enabled", "adapter": adapter_name, "backend": self.adapter.name},
    ...
    def _handle_disable_adapter(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
        adapter_name = arguments.get("adapter_name", "Wi-Fi")
        ok = self.adapter.disable_adapter(adapter_name)
        if ok:
            return DesktopResult.create_success(
                goal=goal, capability=capability, manager=self.name,
                data={"status": "disabled", "adapter": adapter_name, "backend": self.adapter.name},
    ...
    def _handle_release_ip(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
        adapter_name = arguments.get("adapter_name", "")
        ok = self.adapter.release_ip(adapter_name)
        if ok:
            return DesktopResult.create_success(
                goal=goal, capability=capability, manager=self.name,
                data={"status": "ip_released", "adapter": adapter_name, "backend": self.adapter.name},
                events=["ip_released"]
            )
        return DesktopResult.create_failure(
            goal=goal, capability=capability, manager=self.name, error="Failed to release IP lease"
        )

    def _handle_renew_ip(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
        adapter_name = arguments.get("adapter_name", "")
        ok = self.adapter.renew_ip(adapter_name)
        if ok:
            return DesktopResult.create_success(
                goal=goal, capability=capability, manager=self.name,
                data={"status": "ip_renewed", "adapter": adapter_name, "backend": self.adapter.name},
                events=["ip_renewed"]
            )
        return DesktopResult.create_failure(
            goal=goal, capability=capability, manager=self.name, error="Failed to renew IP lease"
        )
    ...
    def _handle_flush_dns(self, goal: str, capability: str) -> DesktopResult:
        ...
        return DesktopResult.create_failure(
            goal=goal, capability=capability, manager=self.name, error="Failed to flush DNS cache"
        )
    ...
    def _handle_disconnect_wifi(self, goal: str, capability: str) -> DesktopResult:
        ...
        return DesktopResult.create_failure(
            goal=goal, capability=capability, manager=self.name, error="Failed to disconnect Wi-Fi"
        )

    def _handle_connect_wifi(self, goal: str, capability: str, arguments: Dict[str, Any]) -> DesktopResult:
        ...
        if ok:
            return DesktopResult.create_success(
                goal=goal, capability=capability, manager=self.name,
                data={"status": "wifi_connected", "ssid": ssid, "backend": self.adapter.name},
                events=["wifi_connected"]
            )
        return DesktopResult.create_failure(
            goal=goal, capability=capability, manager=self.name, error=f"Failed to connect to Wi-Fi SSID '{ssid}'"
        )
