...
class NetworkAdapter(BaseNativeAdapter):
    ...
    @abstractmethod
    def ping(
        self, host: str = "8.8.8.8", count: int = 4, timeout_sec: float = 2.0
    ) -> dict[str, Any]:
        """Ping a target host."""
        raise NotImplementedError
    ...
    @abstractmethod
    def port_check(
        self, host: str = "8.8.8.8", port: int = 80, timeout_sec: float = 2.0
    ) -> dict[str, Any]:
    ...
    def measure_packet_loss(
        self, host: str = "8.8.8.8", count: int = 5
    ) -> dict[str, Any]:
    ...
    @abstractmethod
    def connect_wifi(self, ssid: str, key: str | None = None) -> bool:
    ...
    def get_interfaces(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for adapter in c.Win32_NetworkAdapter():
                ...
                mac = getattr(adapter, "MACAddress", "") or getattr(
                    cfg, "MACAddress", ""
                )
                ...
                interfaces.append(
                    {
                        "name": getattr(adapter, "NetConnectionID", "")
                        or getattr(adapter, "Name", f"Interface {idx}"),
                        ...
                        "dhcp_enabled": (
                            getattr(cfg, "DHCPEnabled", False) if cfg else False
                        ),
                        "backend": self.name,
                    }
                )
            ...
            for cfg in c.Win32_NetworkAdapterConfiguration(IPEnabled=True):
                ...
                if gateways:
                    ...
                    return {
                        ...
                        "dns_servers": list(
                            getattr(cfg, "DNSServerSearchOrder", None) or []
                        ),
                        "backend": self.name,
                    }
        except Exception as e:
...
class NetshNetworkAdapter(NetworkAdapter):
    ...
    def ping(
        ...
        output = self._run_cmd(
            ["ping", "-n", str(count), "-w", str(int(timeout_sec * 1000)), host]
        )
        loss = 100.0
        avg_lat = 0.0
    ...
    def check_internet(self) -> dict[str, Any]:
        ...
        return {
            "connected": connected,
            "latency_ms": res.get("avg_latency_ms", 0),
            "backend": self.name,
        }
...
class PsutilNetworkAdapter(NetworkAdapter):
            ...
            for name, addr_list in addrs.items():
                ...
                if not mac_addrs:
                    mac_addrs = [
                        a.address
                        for a in addr_list
                        if a.family == -1 or "psutil" in str(a.family).lower()
                    ]
    ...
    def ping(
        ...
        ok = self.port_check(host, 80, timeout_sec).get(
            "open", False
        ) or self.port_check(host, 443, timeout_sec).get("open", False)
        return {
            "host": host,
            "success": ok,
    ...
    def measure_latency(self, host: str = "8.8.8.8") -> dict[str, Any]:
        ...
        return {
            "host": host,
            "latency_ms": res.get("avg_latency_ms", 15.0),
            "backend": self.name,
        }
