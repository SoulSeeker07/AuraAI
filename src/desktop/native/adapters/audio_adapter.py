...
class AudioAdapter(BaseNativeAdapter):
    ...
    @abstractmethod
    def get_default_input(self) -> Optional[Dict[str, Any]]:
...
class PyCAWAudioAdapter(AudioAdapter):
    ...
    def get_volume(self) -> Dict[str, Any]:
        try:
            ...
            return {"level": 50.0, "muted": False, "backend": self.name, "error": str(e)}
...
class DummyAudioAdapter(AudioAdapter):
    ...
    def list_devices(self) -> List[Dict[str, Any]]:
        return [
            {"id": "dummy_output", "name": "Mock Default Speakers", "type": "output", "is_default": True},
            {"id": "dummy_input", "name": "Mock Default Microphone", "type": "input", "is_default": True},
        ]
