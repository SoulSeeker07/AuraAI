...
class VerificationLayer:
    ...
    @classmethod
    def verify_window_activated(
        cls, context: NativeExecutionContext
    ) -> VerificationResult:
        """
        ...
        if not window_id:
            return VerificationResult(
                passed=False, error_message="Window ID not provided"
            )
    ...
    def verify_window_closed(
        ...
        return VerificationResult(
            passed=False, error_message=f"Window '{window_id}' still exists"
        )
    ...
    def verify_window_moved(cls, context: NativeExecutionContext) -> VerificationResult:
        ...
        if not expected_x or not expected_y or not window_id:
            return VerificationResult(
                passed=False, error_message="Missing position or window_id"
            )
        ...
        if not target_window:
            return VerificationResult(
                passed=False, error_message=f"Window '{window_id}' not found"
            )
    ...
    def verify_window_resized(
        ...
        if not expected_width or not expected_height or not window_id:
            return VerificationResult(
                passed=False, error_message="Missing dimensions or window_id"
            )
    ...
    def verify_clipboard_updated(
        ...
        if not clipboard or not clipboard.has_text:
            return VerificationResult(
                passed=False, error_message="Clipboard not updated or has no text"
            )
        ...
        return VerificationResult(
            ...
            details={
                ...
                "length": len(clipboard.text) if clipboard.text else 0,
            },
        )

    @classmethod
    def verify_display_updated(
        cls, context: NativeExecutionContext
    ) -> VerificationResult:
        """
        ...
        if not context.result or not context.result.success:
            return VerificationResult(
                passed=False, error_message="Failed to retrieve display information"
            )
        ...
        return VerificationResult(passed=True, details={"display_count": len(displays)})

    @classmethod
    def verify_power_operation(
        cls, context: NativeExecutionContext
    ) -> VerificationResult:
        """
        ...
        if not context.result or not context.result.success:
            return VerificationResult(
                passed=False, error_message="Power operation failed"
            )

        return VerificationResult(passed=True)

    @classmethod
    def verify_audio_device_updated(
        cls, context: NativeExecutionContext
    ) -> VerificationResult:
        """
        ...
        if not context.result or not context.result.success:
            return VerificationResult(
                passed=False, error_message="Failed to retrieve audio devices"
            )

        devices = context.desktop_context.get_audio_devices()
        if not devices or len(devices) == 0:
            return VerificationResult(
                passed=False, error_message="No audio devices found"
            )

        return VerificationResult(passed=True, details={"device_count": len(devices)})

    @classmethod
    def verify_network_interface_updated(
        cls, context: NativeExecutionContext
    ) -> VerificationResult:
        """
        ...
        if not context.result or not context.result.success:
            return VerificationResult(
                passed=False, error_message="Failed to retrieve network interfaces"
            )

        interfaces = context.desktop_context.get_network_interfaces()
        if not interfaces or len(interfaces) == 0:
            return VerificationResult(
                passed=False, error_message="No network interfaces found"
            )

        return VerificationResult(
            passed=True, details={"interface_count": len(interfaces)}
        )

    @classmethod
    def verify_registry_key_updated(
        cls, context: NativeExecutionContext
    ) -> VerificationResult:
        """
        ...
        if not context.result or not context.result.success:
            return VerificationResult(
                passed=False, error_message="Failed to read registry key"
            )
    ...
    def verify_service_updated(
        ...
        if not context.result or not context.result.success:
            return VerificationResult(
                passed=False, error_message="Service operation failed"
            )
    ...
    def verify(
        ...
        mode: VerificationMode = VerificationMode.POST_CHECK,
    ) -> VerificationResult:
        """
        ...
        if mode in [VerificationMode.POST_CHECK, VerificationMode.BOTH]:
            ...
            if context.result.data is not None:
                return VerificationResult(
                    ...
                    details={
                        "has_data": True,
                        "data_type": type(context.result.data).__name__,
                    },
                )
    ...
    def register_default_handlers(cls) -> None:
        ...
        cls.register_handler(
            "list_network_interfaces", cls.verify_network_interface_updated
        )
        cls.register_handler(
            "get_default_interface", cls.verify_network_interface_updated
        )
