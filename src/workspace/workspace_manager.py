...
class WorkspaceManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Workspace Manager initialized with features: {list(self.enabled_features.keys())}"
        )
    ...
    async def update(self):
            ...
            try:
                ...
                if git_repo:
                    ...
                    if (
                        ...
                        and self.state.current_project.git_repo is None
                    ):
        ...
        logger.debug(
            f"Workspace state updated: {len(self.state.open_files)} open files"
        )
    ...
    async def update_project(self, project_path: str | None = None):
        """
    ...
    def _save_state(self):
        ...
        try:
            ...
            if self.state.clipboard:
                state_dict["clipboard"][
                    "timestamp"
                ] = self.state.clipboard.timestamp.isoformat()
    ...
    def _load_state(self):
        ...
        try:
            ...
            if self.state.clipboard:
                self.state.clipboard.timestamp = datetime.fromisoformat(
                    clipboard_data["timestamp"]
                )
