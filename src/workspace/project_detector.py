...
from .models import CurrentProject, ProjectType, GitRepository
...
class ProjectDetectionResult:
    ...
    project: Optional[CurrentProject]
    detected_at: str
    method: str
...
class ProjectDetector:
    ...
    PYTHON_MARKERS = {
        ...
        'py.typed', 'tox.ini', 'poetry.lock', 'Pipfile.lock'
    }

    NODE_MARKERS = {
        'package.json', 'node_modules', 'yarn.lock', 'pnpm-lock.yaml'
    }

    DOTNET_MARKERS = {
        '.csproj', '.sln', '.fsproj', '.vbproj', 'packages.config'
    }
    ...
    async def detect_current_project(self, project_path: Optional[str] = None) -> Optional[CurrentProject]:
        """
        ...
        try:
            ...
            while current_path != current_path.parent:
                ...
                if result.project:
                    ...
                    logger.info(f"Detected project: {result.project.name} at {result.project.path}")
    ...
    def _check_git_repo(self, path: Path) -> Optional[GitRepository]:
        """
                                ...
                                try:
                                    with open(remote_file, 'r') as f:
                                        ...
                                        for line in config_content.split('\n'):
                                            if 'url = ' in line:
                                                remote_url = line.split('url = ')[-1].strip()
                                                break
                                except Exception:
                                    pass
                        else:
    ...
    async def detect_project_by_path(self, path: str) -> Optional[CurrentProject]:
        """
