...
class STTEngine(ABC):
    ...
    def __init__(self, settings: STTSettings):
        ...
        self._stream = None
        self._partial_callback: Optional[Callable[[str, float], None]] = None
        self._final_callback: Optional[Callable[[str, float], None]] = None
    ...
    def set_callbacks(self, partial: Callable[[str, float], None], final: Callable[[str, float], None], error: Callable[[str], None]) -> None:
    ...
    def finalize(self) -> str:
        ...
        try:
            ...
            if result:
                ...
                duration = self._total_duration + len(self._stream) / self.settings.sample_rate / 2
                self._emit_final(text, duration)
                return text
