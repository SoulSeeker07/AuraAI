...
class InterruptionManager:
    ...
    def __init__(
        ...
        self.interrupt_start_time: datetime | None = None
        self.interrupt_reason: InterruptionReason | None = None
        self.current_interruption_count = 0
        self.total_interruptions = 0

        # Callbacks
        self.on_interrupt_start: Callable[[InterruptionReason], None] | None = None
        self.on_interrupt_end: Callable[[], None] | None = None
    ...
    def start_interrupt(self, reason: InterruptionReason) -> None:
        ...
        logger.info(
            f"Interruption started: {reason.value} (count: {self.current_interruption_count})"
        )
    ...
    def end_interrupt(self) -> None:
        ...
        self.state = InterruptionState.IDLE
        logger.info(
            f"Interruption ended (total interrupted time: {self.total_interrupt_time:.2f}s)"
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "interrupt_start_time": (
                self.interrupt_start_time.isoformat()
                if self.interrupt_start_time
                else None
            ),
            "interrupt_reason": (
                self.interrupt_reason.value if self.interrupt_reason else None
            ),
            "current_interruption_count": self.current_interruption_count,
            "total_interruptions": self.total_interruptions,
            "total_interrupt_time": self.total_interrupt_time,
