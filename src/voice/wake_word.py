...
class WakeWordEngine(ABC):
    ...
    def __init__(self, sensitivity: float = 0.5, phrase_list: list[str] | None = None):
        """
        ...
        logger.info(
            f"Wake word engine initialized with {len(self.phrase_list)} phrases"
        )
...
class OpenWakeWord(WakeWordEngine):
    ...
    def __init__(self, phrase_list: list[str] | None = None, sensitivity: float = 0.5):
        """
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.model = openwakeword.AudioModel(
                model_path=None, pretrained=True  # Use default model
            )
            ...
            if predictions:
                for keyword, probability in predictions.items():
                    if probability > self.sensitivity and keyword in self.phrase_list:
                        logger.info(
                            f"Wake word detected: {keyword} ({probability:.2f})"
                        )
...
class WakeWordManager:
    ...
    def __init__(
        ...
        # Engine instances
        self.engine: WakeWordEngine | None = None
        ...
        try:
            ...
            if self.provider == WakeWordProvider.AURA:
                ...
                self.engine = OpenWakeWord(
                    phrase_list=self.phrase_list, sensitivity=self.sensitivity
                )
            else:  # LOCAL
    ...
    def _get_access_key(self) -> str | None:
        """
