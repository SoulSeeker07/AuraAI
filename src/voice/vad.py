...
class VoiceActivityDetector:
    ...
    def __init__(
        ...
        self.silence_buffer = deque(maxlen=int(silence_threshold / (window_size_ms / 1000.0)))
        ...
        self.on_speech_detected: Optional[Callable[[float], None]] = None  # Pass speech duration
    ...
    def _process_speech_start(self, audio: np.ndarray) -> Tuple[VADState, float]:
        ...
        if self.mode == VADMode.SILENCE:
            ...
            should_end = self._check_silence(audio) or self.current_energy < self.energy_threshold
    ...
    def _finalize_speech(self):
        ...
        if speech_duration >= self.min_speech_duration:
            ...
            logger.info(f"Speech detected: {speech_duration:.2f}s (count: {self.speech_count})")
