...
class ContentFetcher:
    ...
    def fetch(self, url: str) -> Optional[Document]:
        ...
        try:
            ...
            # Parse HTML
            return self._parse_response(url, response.text, response.headers.get("Content-Type", ""))
    ...
    def fetch_summary(self, url: str, max_length: int = 300) -> Optional[str]:
        """
    ...
    def _extract_date(self, soup: BeautifulSoup, content_type: str) -> Optional[datetime]:
        ...
        if "documentation" in content_type.lower():
            text = soup.get_text()
            match = re.search(r'(updated|modified)\s+(?:on\s+)?(\w{3},?\s+\d{1,2},?\s+\d{4})', text, re.IGNORECASE)
            if match:
                return self._parse_date(match.group(2))

        return None

    def _determine_content_type(self, url: str, content_type: str, soup: BeautifulSoup) -> str:
        """
