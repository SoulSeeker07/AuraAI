...
from .research_plan import (
    ResearchPlan, ResearchStep, StopCondition, StopReason, ResearchMode
)
...
class ResearchPlanner:
    ...
    def create_plan(self, query: str, mode: ResearchMode = ResearchMode.STANDARD) -> ResearchPlan:
        ...
        logger.info(f"Created plan {plan_id} with {len(steps)} steps for query: {query}")
    ...
    def _analyze_query(self, query: str, mode: ResearchMode) -> Dict[str, Any]:
        ...
        if any(word in query_lower for word in ["compare", "vs", "versus", "difference"]):
            query_type = "compare"
        elif any(word in query_lower for word in ["what is", "who is", "how do", "explain", "define"]):
            query_type = "entity"
        elif any(word in query_lower for word in ["features", "benefits", "advantages", "limitations"]):
            query_type = "aspect"
        elif any(word in query_lower for word in ["summary", "overview", "introduction"]):
            query_type = "summary"
        ...
        if any(word in query_lower for word in ["current", "latest", "today", "recent"]):
            freshness = "hour"
    ...
    def _estimate_complexity(self, query: str) -> int:
        ...
        complexity += len([word for word in query.lower().split() if word in ["what", "why", "how", "compare", "difference"]])

        return min(5, complexity)

    def _decompose_query(self, query: str, analysis: Dict[str, Any], mode: ResearchMode) -> List[ResearchStep]:
        """
        ...
        if query_type == "compare":
            ...
            if entities:
                for i, entity in enumerate(entities[:2]):  # Max 2 entities for comparison
                    step_id = get_step_id()
                    step = ResearchStep(
                        step_id=step_id,
    ...
    def _extract_entities(self, query: str) -> List[str]:
        ...
        entities = [word for word in words if len(word) > 3 and not word in ["is", "the", "a", "an", "of"]]
    ...
    def generate_next_plan_iteration(self, current_plan: ResearchPlan, evidence: Any) -> Optional[ResearchPlan]:
        """
        ...
        new_steps = []
        step_id_counter = sum(step.step_id for step in current_plan.steps) if current_plan.steps else 0
